...
export function MiniGameModal({
  ...
  const [junkyardDifficulty, setJunkyardDifficulty] = useState<JunkyardDifficulty>('standard')
  const challengeGate = useGameStore((s) => s.storyChapters['storm-dock']?.completedOnce)

  useEffect(() => {
    setJunkyardDifficulty(quest.junkyardDifficulty ?? 'standard')
  }, [quest.id, quest.junkyardDifficulty])
              ...
              <div className="rounded-xl border border-slate-700/80 bg-slate-900/40 p-3">
                ...
                {junkyardBlocked ? (
                  <p className="mt-2 text-xs text-amber-200/90">
                    Challenge unlocks after you clear Storm Dock in World.
                  </p>
                ) : null}
              </div>
            ) : null}
